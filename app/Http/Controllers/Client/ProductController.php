...
class ProductController extends Controller
{
    ...
    {
        $data['list'] = Product::where(['status'=>1])->orderBy('id','DESC')->select('id','description','category','name','discount','price','images','slug','cate_slug','type_slug')
        ->paginate(12);
        $data['title'] = "Tất cả các khoá";
    ...
    public function detail_product($cate,$slug)
    {
        ...
        $data['productlq'] = Product::where(['status'=>1,'cate_slug'=>$cate])->paginate(6);
...
// return;
    ...
    public function getProductTypeTab(Request $request)
    {
        $id = $request->id;
        $typeProduct = TypeProduct::findOrFail($id);
        $data['products'] = Product::where(['status'=>1, 'discountStatus'=>1, 'type_slug'=>$typeProduct->slug])->limit(8)->get(['id', 'name', 'slug', 'images', 'cate_slug']);
        $html = view('layouts.product.ajax-type-product', $data)->render();
